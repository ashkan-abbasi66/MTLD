...
% 
function [out,runtime] = LASSC_MS(imn,sigma,varargin)
...
%         [out.ANh,~] = TLdenoising_noClip(AN,sigma,n,sparsity_controller);
        if sigma <= 15
            out.ANh = run_LASSC_denoising_015(AN,sigma,1);
        elseif sigma <=25
            % 2nd exp.
            out.ANh = run_LASSC_denoising(AN,sigma,1);
        else
            out.ANh = run_LASSC_denoising_org(AN,sigma);
            % compare with "run_LASSC_denoising_050" for "barbara"
        end
...
%     [H_hat,~] = TLdenoising_noClip(H,sigma,n,sparsity_controller);
    H_hat = run_LASSC_denoising(H,sigma,2);
    H_time = toc;
...
%     [V_hat,~] = TLdenoising_noClip(V,sigma,n,sparsity_controller);
    V_hat = run_LASSC_denoising(V,sigma,2);
    V_time = toc;
...
%     [D_hat,~] = TLdenoising_noClip(D,sigma,n,sparsity_controller);
    D_hat = run_LASSC_denoising(D,sigma,2);
    D_time = toc;
